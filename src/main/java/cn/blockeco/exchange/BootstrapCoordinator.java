...
final class BootstrapCoordinator<T> {
    ...
    void coordinate(CompletionStage<T> background) { background.whenComplete((value, error) -> { if (stopped.get()) { clean(value); return; } main.submit(() -> { if (error != null) { fail(error); return null; } try { if (stopped.get() || !ready.apply(value)) { clean(value); fail(new IllegalStateException("bootstrap wiring failed")); } else accepting.set(true); } catch (Throwable failure) { clean(value); fail(failure); } return null; }); }); }
