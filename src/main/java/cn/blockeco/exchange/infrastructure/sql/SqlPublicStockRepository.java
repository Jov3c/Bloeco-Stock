...
public final class SqlPublicStockRepository implements PublicStockRepository {
    ...
    @Override public Optional<PublicStockInfo> findInfo(String query) {
        String sql="SELECT c.display_name,c.status,c.total_shares,sl.stock_code,sl.issue_reference_price_minor,bc.model_price_minor,bc.quotes_paused,be.headline,be.body,be.starts_at,bc.industry FROM companies c LEFT JOIN stock_listings sl ON sl.company_id=c.id LEFT JOIN bluechip_companies bc ON bc.company_id=c.id LEFT JOIN bluechip_events be ON (be.company_id=c.id OR be.industry=bc.industry OR be.scope='MARKET') AND be.state='ACTIVE' AND be.ends_at > ? WHERE c.normalized_name=? OR sl.stock_code=? ORDER BY CASE WHEN sl.stock_code=? THEN 0 ELSE 1 END LIMIT 1";
        String name=normalized(query),code=code(query); try(Connection c=dataSource.getConnection();PreparedStatement s=c.prepareStatement(sql)){s.setString(1,Instant.now().toString());s.setString(2,name);s.setString(3,code);s.setString(4,code);try(ResultSet r=s.executeQuery()){if(!r.next())return Optional.empty();String stockCode=r.getString(4),headline=r.getString(8),body=r.getString(9),started=r.getString(10); long price=r.getLong(5); Optional<Money> reference=r.wasNull()?Optional.empty():Optional.of(Money.ofMinor(price)); long model=r.getLong(6); Optional<MarketNewsItem> event=headline==null?Optional.empty():Optional.of(new MarketNewsItem(headline,body,Instant.parse(started))); Optional<PublicMarketState> state=r.wasNull()?Optional.empty():Optional.of(new PublicMarketState(Money.ofMinor(model),r.getInt(7)!=0,event));return Optional.of(new PublicStockInfo(r.getString(1),Optional.ofNullable(stockCode),CompanyStatus.valueOf(r.getString(2)),reference,r.getLong(3),state,Optional.ofNullable(r.getString(11))));}}catch(SQLException e){throw failed("find public stock",e);}
    }
    ...
    @Override public List<MarketNewsItem> recentNews(int requestedLimit) { String sql="SELECT headline,body,published_at FROM (SELECT headline,body,starts_at published_at FROM bluechip_events UNION ALL SELECT '分红公告',body,created_at FROM company_announcements WHERE body LIKE 'DIVIDEND_PAID:%' OR body LIKE 'NO_DIVIDEND:%' UNION ALL SELECT CASE event_type WHEN 'BLUECHIP_LIQUIDITY_DEGRADED' THEN '流动性提示' ELSE '流动性恢复' END,payload_json,occurred_at FROM audit_events WHERE event_type IN ('BLUECHIP_LIQUIDITY_DEGRADED','BLUECHIP_LIQUIDITY_RESTORED')) ORDER BY published_at DESC LIMIT ?";try(Connection c=dataSource.getConnection();PreparedStatement s=c.prepareStatement(sql)){s.setInt(1,limit(requestedLimit));try(ResultSet r=s.executeQuery()){List<MarketNewsItem> result=new ArrayList<>();while(r.next())result.add(new MarketNewsItem(r.getString(1),r.getString(2),Instant.parse(r.getString(3))));return List.copyOf(result);}}catch(SQLException e){throw failed("list market news",e);} }
