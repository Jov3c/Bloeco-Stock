...
public final class SqlPublicStockRepository implements PublicStockRepository {
    ...
    @Override public List<PublicMarketRow> market() {
        String sql="SELECT MIN(occurred_at),MAX(occurred_at) FROM stock_trades";
        Instant[] bounds;
        try(Connection c=dataSource.getConnection();PreparedStatement s=c.prepareStatement(sql);ResultSet r=s.executeQuery()){if(!r.next()||r.getString(1)==null)bounds=new Instant[]{Instant.EPOCH,Instant.EPOCH.plusSeconds(1)};else { Instant first=Instant.parse(r.getString(1)),last=Instant.parse(r.getString(2)); bounds=new Instant[]{first,last.equals(Instant.MAX)?Instant.MAX:last.plusNanos(1)}; }}catch(SQLException e){throw failed("bound public market",e);}
    ...
    private static MarketFacts marketFacts(Connection c,String company,long reference,Instant start,Instant next)throws SQLException{long latest=reference,previous=reference,volume=0,turnover=0;String sql="SELECT price_minor,shares,notional_minor,occurred_at FROM stock_trades WHERE company_id=? ORDER BY occurred_at,id";try(PreparedStatement s=c.prepareStatement(sql)){s.setString(1,company);try(ResultSet r=s.executeQuery()){while(r.next()){Instant at=Instant.parse(r.getString(4));long price=r.getLong(1);latest=price;if(at.isBefore(start))previous=price;if(!at.isBefore(start)&&at.isBefore(next)){volume=Math.addExact(volume,r.getLong(2));turnover=Math.addExact(turnover,r.getLong(3));}}}}return new MarketFacts(latest,previous,volume,turnover);}
