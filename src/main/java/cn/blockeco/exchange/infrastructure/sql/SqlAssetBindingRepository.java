...
public final class SqlAssetBindingRepository implements AssetBindingRepository {
    ...
    private Optional<AssetBinding> find(String sql,String...v){try(Connection c=dataSource.getConnection();PreparedStatement s=c.prepareStatement(sql)){for(int i=0;i<v.length;i++)s.setString(i+1,v[i]);try(ResultSet r=s.executeQuery()){return r.next()?Optional.of(new AssetBinding(UUID.fromString(r.getString("id")),new CompanyId(UUID.fromString(r.getString("company_id"))),r.getString("adapter_id"),r.getString("external_key"),UUID.fromString(r.getString("verified_owner_uuid")),AssetBindingState.valueOf(r.getString("state")),Instant.parse(r.getString("created_at")))):Optional.empty();}}catch(SQLException e){throw new IllegalStateException("could not read asset binding",e);}}
}
