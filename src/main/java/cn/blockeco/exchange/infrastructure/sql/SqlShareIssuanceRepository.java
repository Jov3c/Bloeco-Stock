...
public final class SqlShareIssuanceRepository implements ShareIssuanceRepository {
    ...
    private static void creditHolding(Connection c, String company, UUID holder, long shares) throws SQLException { try (PreparedStatement read = c.prepareStatement("SELECT available_shares FROM share_holdings WHERE company_id=? AND holder_uuid=?")) { read.setString(1, company); read.setString(2, holder.toString()); try (ResultSet rows = read.executeQuery()) { if (!rows.next()) { try (PreparedStatement insert = c.prepareStatement("INSERT INTO share_holdings (company_id,holder_uuid,available_shares,reserved_shares) VALUES (?,?,?,0)")) { insert.setString(1, company); insert.setString(2, holder.toString()); insert.setLong(3, shares); insert.executeUpdate(); } return; } long available = rows.getLong(1); long updated = Math.addExact(available, shares); try (PreparedStatement update = c.prepareStatement("UPDATE share_holdings SET available_shares=? WHERE company_id=? AND holder_uuid=? AND available_shares=?")) { update.setLong(1, updated); update.setString(2, company); update.setString(3, holder.toString()); update.setLong(4, available); if (update.executeUpdate() != 1) throw new IllegalStateException("holding changed during issuance settlement"); } } } }
    ...
    private static void updateCompany(Connection c, String company, long issued, long raised, Instant at) throws SQLException { long oldTotal = scalar(c, "SELECT total_shares FROM companies WHERE id=?", company); long oldCash = scalar(c, "SELECT cash_minor FROM company_cash_accounts WHERE company_id=?", company); long oldCapital = scalar(c, "SELECT paid_in_capital_minor FROM company_cash_accounts WHERE company_id=?", company); long total = Math.addExact(oldTotal, issued), cash = Math.addExact(oldCash, raised), capital = Math.addExact(oldCapital, raised); try (PreparedStatement companies = c.prepareStatement("UPDATE companies SET total_shares=? WHERE id=?"); PreparedStatement account = c.prepareStatement("UPDATE company_cash_accounts SET cash_minor=?,paid_in_capital_minor=? WHERE company_id=? AND cash_minor=? AND paid_in_capital_minor=?"); PreparedStatement listing = c.prepareStatement("UPDATE stock_listings SET issued_shares=? WHERE company_id=?")) { companies.setLong(1, total); companies.setString(2, company); companies.executeUpdate(); account.setLong(1, cash); account.setLong(2, capital); account.setString(3, company); account.setLong(4, oldCash); account.setLong(5, oldCapital); if (account.executeUpdate()!=1) throw new IllegalStateException("company cash account changed during issuance settlement"); listing.setLong(1, total); listing.setString(2, company); if (listing.executeUpdate()!=1) throw new IllegalStateException("stock listing missing"); } ledger(c, "COMPANY_TREASURY", company, null, raised, at); }
