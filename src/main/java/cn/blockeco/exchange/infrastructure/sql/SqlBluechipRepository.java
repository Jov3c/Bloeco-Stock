...
public final class SqlBluechipRepository implements BluechipRepository {
    ...
    @Override public List<IntradayTrade> sessionTrades(CompanyId companyId, Instant start, Instant end) {
        try (Connection c = dataSource.getConnection(); PreparedStatement s = c.prepareStatement("SELECT occurred_at,price_minor,shares FROM stock_trades WHERE company_id=? AND occurred_at>=? AND occurred_at<? ORDER BY occurred_at,id")) {
            s.setString(1, companyId.value().toString()); s.setString(2, start.toString()); s.setString(3, end.toString());
            try (ResultSet r = s.executeQuery()) { var result = new java.util.ArrayList<IntradayTrade>(); while (r.next()) result.add(new IntradayTrade(Instant.parse(r.getString(1)), Money.ofMinor(r.getLong(2)), r.getLong(3))); return List.copyOf(result); }
        } catch (SQLException e) { throw new IllegalStateException("could not read intraday market trades", e); }
    }
