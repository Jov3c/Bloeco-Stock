...
public final class SecuritiesCashService {
    ...
    private CompletionStage<SecuritiesCashResult> depositLegOne(SecuritiesCashOperation o) {
        return gateway.withdrawPlayer(o.playerId(),o.amount()).handle((r,t)->new External(r,t)).thenCompose(x -> {
            ...
            return sql(() -> { transition(o,SecuritiesCashOperationState.PREPARED,SecuritiesCashOperationState.PLAYER_WITHDRAWN,SecuritiesCashOperationState.PLAYER_WITHDRAWN,"player withdrawal confirmed"); return null; })
                ...
                .thenCompose(y -> { if(!success(y)) return afterFailure(o,SecuritiesCashOperationState.PLAYER_WITHDRAWN,SecuritiesCashOperationState.PLAYER_WITHDRAWN,y,true);
                    return sql(() -> { SecuritiesCashOperation latest=require(o.id()); transactions.inTransaction(c->{repository.transitionOperation(c,o.id(),SecuritiesCashOperationState.PLAYER_WITHDRAWN,SecuritiesCashOperationState.ESCROW_DEPOSITED,SecuritiesCashOperationState.ESCROW_DEPOSITED,"escrow deposit confirmed",clock.now()); repository.completeDeposit(c,latest,clock.now());return null;}); return new SecuritiesCashResult(o.id(),SecuritiesCashOperationState.COMPLETED,"completed");}); });
        });
    }
    ...
    private void transition(SecuritiesCashOperation o,SecuritiesCashOperationState expected,SecuritiesCashOperationState state,SecuritiesCashOperationState stage,String detail){transactions.inTransaction(c->{repository.transitionOperation(c,o.id(),expected,state,stage,detail,clock.now());return null;});}
