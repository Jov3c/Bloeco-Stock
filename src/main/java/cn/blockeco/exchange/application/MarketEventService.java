...
public final class MarketEventService {
    ...
    private BluechipEvent draft(String scope,String code,String industry,int impact,Instant now,Duration duration){ var company="COMPANY".equals(scope)?repository.findByStockCode(code).orElseThrow(()->new IllegalArgumentException("unknown bluechip: "+code)):null; String target=company==null?industry:company.listing().stockCode(); return new BluechipEvent(UUID.randomUUID().toString(),scope,company==null?null:company.companyId().value().toString(),industry,"BlockStock 模拟快讯："+target+" 物流挑战","这是 BlockStock 世界中的模拟经营事件，不代表现实新闻。",impact,impact/2,now,now.plus(duration),"ACTIVE"); }
