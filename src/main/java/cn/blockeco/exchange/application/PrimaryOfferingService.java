...
public class PrimaryOfferingService {
 ...
 public CompletionStage<SubscriptionResult> subscribe(UUID subscriber,UUID offeringId,long shares){
  ...
  return CompletableFuture.supplyAsync(()->{PrimaryOffering offering=offerings.find(offeringId).orElseThrow(()->new IllegalArgumentException("offering not found"));UUID id=UUID.nameUUIDFromBytes((offeringId+":"+subscriber+":"+shares).getBytes(StandardCharsets.UTF_8));return transactions.inTransaction(c->offerings.prepareSubscription(c,id,offering,subscriber,shares,clock.now()));},executor)
   .handle((prepared,error)->{if(error!=null)return CompletableFuture.completedFuture(SubscriptionResult.of(SubscriptionResult.Status.NOT_OPEN));TreasuryOperation op=prepared.operation();if(op.state().name().equals("COMPLETED"))return CompletableFuture.completedFuture(SubscriptionResult.of(SubscriptionResult.Status.SUCCESS));if(!prepared.newlyPrepared())return CompletableFuture.completedFuture(SubscriptionResult.of(SubscriptionResult.Status.RECOVERY_REQUIRED));return CompletableFuture.supplyAsync(()->escrow.withdrawPlayer(subscriber,op.amount(),op.id()),executor).thenCompose(result->{if(result.outcome()!=EconomyGateway.Outcome.SUCCESS)return CompletableFuture.supplyAsync(()->transactions.inTransaction(c->{offerings.cancelPrepared(c,op.id(),clock.now());return result.outcome()==EconomyGateway.Outcome.INSUFFICIENT_FUNDS?SubscriptionResult.of(SubscriptionResult.Status.INSUFFICIENT_FUNDS):SubscriptionResult.of(SubscriptionResult.Status.PROVIDER_FAILURE);}),executor);return CompletableFuture.supplyAsync(()->{transactions.inTransaction(c->{offerings.markWithdrawn(c,op.id(),clock.now());return null;});return escrow.depositEscrow(op.amount(),op.id());},executor).thenCompose(deposit->{if(deposit.outcome()!=EconomyGateway.Outcome.SUCCESS)return ambiguous(op,"PLAYER_WITHDRAWN");return CompletableFuture.supplyAsync(()->transactions.inTransaction(c->{offerings.markEscrowDeposited(c,op.id(),clock.now());offerings.completeSubscription(c,op.id(),clock.now());return SubscriptionResult.of(SubscriptionResult.Status.SUCCESS);}),executor).exceptionallyCompose(e->ambiguous(op,"ESCROW_DEPOSITED"));});});}).thenCompose(x->x);
 }
 private CompletionStage<SubscriptionResult> ambiguous(TreasuryOperation op,String expected){return CompletableFuture.supplyAsync(()->transactions.inTransaction(c->{offerings.markAmbiguous(c,op.id(),expected,clock.now());return SubscriptionResult.of(SubscriptionResult.Status.RECOVERY_REQUIRED);}),executor);}
