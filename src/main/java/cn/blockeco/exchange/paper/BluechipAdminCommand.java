...
public final class BluechipAdminCommand implements CommandExecutor, TabCompleter {
    ...
    private final Initializer initializer; private final PauseControl quotes; private final FundControl fund; private final EventControl events; private final Messages messages; private final List<String> industries; private final QuantStatus quant;
    public BluechipAdminCommand(Initializer initializer, PauseControl quotes, FundControl fund, EventControl events, Messages messages) { this(initializer, quotes, fund, events, messages, List.of(), code -> "量化状态暂不可用。"); }
    public BluechipAdminCommand(Initializer initializer, PauseControl quotes, FundControl fund, EventControl events, Messages messages, List<String> industries) { this(initializer, quotes, fund, events, messages, industries, code -> "量化状态暂不可用。"); }
    public BluechipAdminCommand(Initializer initializer, PauseControl quotes, FundControl fund, EventControl events, Messages messages, List<String> industries, QuantStatus quant) { this.initializer=Objects.requireNonNull(initializer);this.quotes=Objects.requireNonNull(quotes);this.fund=Objects.requireNonNull(fund);this.events=Objects.requireNonNull(events);this.messages=Objects.requireNonNull(messages);this.industries=List.copyOf(Objects.requireNonNull(industries));this.quant=Objects.requireNonNull(quant); }
    @Override public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        try { switch (args[1].toLowerCase(Locale.ROOT)) {
            ...
            case "quant" -> { if(args.length < 3 || !"status".equalsIgnoreCase(args[2]) || args.length > 4){usage(sender);break;} sender.sendMessage(messages.result(quant.describe(args.length == 4 ? args[3].toUpperCase(Locale.ROOT) : null))); }
            case "fund" -> { if(args.length!=5){usage(sender);break;} long value=Long.parseLong(args[4]); if(value==0){sender.sendMessage(messages.result("资金调整不能为零。"));break;} String kind=args[3].toLowerCase(Locale.ROOT); if("cash".equals(kind)){sender.sendMessage(messages.result("为保证资金真实托管，蓝筹现金不支持管理员调整。"));break;} fund.adjust(args[2].toUpperCase(Locale.ROOT),kind,value);sender.sendMessage(messages.result("蓝筹可用股票已调整。")); }
    ...
    @Override public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) { if(!sender.hasPermission(PERMISSION))return List.of(); if(args.length==1)return filter(List.of("bluechip"),args[0]); if(args.length==2&&"bluechip".equalsIgnoreCase(args[0]))return filter(List.of("init","pause","resume","fund","event","quant"),args[1]); if(args.length==3&&"quant".equalsIgnoreCase(args[1]))return filter(List.of("status"),args[2]); if(args.length==4&&"fund".equalsIgnoreCase(args[1]))return filter(List.of("cash","shares"),args[3]); if(args.length==3&&"event".equalsIgnoreCase(args[1]))return filter(List.of("market","industry"),args[2]); if(args.length==4&&"event".equalsIgnoreCase(args[1])&&"industry".equalsIgnoreCase(args[2]))return filter(industries,args[3]); return List.of(); }
    private void usage(CommandSender sender){sender.sendMessage(messages.result("用法：/stockadmin bluechip <init|pause|resume|quant status [代码]|fund <代码> shares <值>|event <代码|market> <影响万分比>|event industry <行业> <影响万分比>>；现金由托管资金自动管理。"));}
    ...
    @FunctionalInterface public interface QuantStatus { String describe(String stockCode); }
}
