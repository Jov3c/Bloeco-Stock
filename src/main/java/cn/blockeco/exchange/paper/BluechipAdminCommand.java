...
public final class BluechipAdminCommand implements CommandExecutor, TabCompleter {
    ...
    private final Initializer initializer; private final PauseControl quotes; private final FundControl fund; private final EventControl events; private final Messages messages;
    public BluechipAdminCommand(Initializer initializer, PauseControl quotes, FundControl fund, EventControl events, Messages messages) { this.initializer=Objects.requireNonNull(initializer);this.quotes=Objects.requireNonNull(quotes);this.fund=Objects.requireNonNull(fund);this.events=Objects.requireNonNull(events);this.messages=Objects.requireNonNull(messages); }
    @Override public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        try { switch (args[1].toLowerCase(Locale.ROOT)) {
            ...
            case "event" -> { if(args.length!=4){usage(sender);break;} int impact=Integer.parseInt(args[3]); events.trigger(args[2],impact);sender.sendMessage(messages.result("蓝筹事件已触发。")); }
