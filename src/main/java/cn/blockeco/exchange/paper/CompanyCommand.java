...
public final class CompanyCommand implements CommandExecutor, CommandAcceptanceGate {
    ...
    private boolean create(CommandSender sender, String[] args) {
        ...
        registration.register(request.get()).whenComplete((result, failure) -> mainThread.submit(() -> {
            ...
            if (online(player)) player.sendMessage(failure == null ? resultMessage(result) : messages.registrationFailed());
            return null;
        }));
        return true;
    }
    private boolean info(CommandSender sender, String[] args) {
        ...
        queries.findByName(String.join(" ", Arrays.copyOfRange(args, 1, args.length))).whenComplete((company, failure) -> mainThread.submit(() -> { if (online(sender)) sender.sendMessage(failure != null ? messages.lookupFailed() : company.map(c -> messages.companyInfo(c.displayName(), c.status())).orElse(messages.companyNotFound())); return null; }));
        return true;
    }
    ...
    private static boolean online(CommandSender sender) { return !(sender instanceof Player player) || player.isOnline(); }
}
