...
public final class CompanyTabCompleter implements TabCompleter {
    ...
    public List<String> complete(CommandSender sender, String[] args) {
        ...
        if (args.length == 2 && "ipo".equalsIgnoreCase(args[0])) { java.util.ArrayList<String> choices=new java.util.ArrayList<>(List.of("list","info")); if(sender instanceof Player&&sender.hasPermission("blockeco.company.ipo.announce"))choices.add("announce"); if(sender instanceof Player&&sender.hasPermission("blockeco.company.ipo.subscribe"))choices.add("subscribe"); return filter(choices,args[1]); }
        return List.of();
    }
