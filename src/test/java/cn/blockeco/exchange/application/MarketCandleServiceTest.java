...
class MarketCandleServiceTest {
    ...
    @Test void candleUsesActualTradeForAllOhlcValuesInsteadOfModelFallback() throws Exception {
        ...
        try (var database = new Database("jdbc:sqlite:" + file)) {
            ...
            new BluechipBootstrapService(config(), UUID.fromString("00000000-0000-0000-0000-000000000099"), new SqlCompanyRepository(database.dataSource()), new SqlStockListingRepository(database.dataSource()), repository, database, Runnable::run, () -> now).initializeMissing().toCompletableFuture().join();
