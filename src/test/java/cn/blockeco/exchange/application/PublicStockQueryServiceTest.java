...
class PublicStockQueryServiceTest {
    ...
    @Test void market_uses_real_trades_for_shanghai_day_without_mutating_facts() throws Exception {
        ...
        try(Database db=new Database("jdbc:sqlite:"+file)){db.migrate(); CompanyId listed=Fixtures.company(db,100); db.inTransaction(c->{try(var u=c.prepareStatement("UPDATE companies SET normalized_name='listed facts' WHERE id=?")){u.setString(1,listed.value().toString());u.executeUpdate();}return null;}); CompanyId hidden=Fixtures.company(db,100); UUID buyer=UUID.randomUUID(),seller=UUID.randomUUID();
            ...
            UUID oldBuy=UUID.randomUUID(),oldSell=UUID.randomUUID(),firstBuy=UUID.randomUUID(),firstSell=UUID.randomUUID(),laterBuy=UUID.randomUUID(),laterSell=UUID.randomUUID(),boundaryBuy=UUID.randomUUID(),boundarySell=UUID.randomUUID();
            db.inTransaction(c->{addOrder(c,oldBuy,listed,buyer,"BUY");addOrder(c,oldSell,listed,seller,"SELL");addOrder(c,firstBuy,listed,buyer,"BUY");addOrder(c,firstSell,listed,seller,"SELL");addOrder(c,laterBuy,listed,buyer,"BUY");addOrder(c,laterSell,listed,seller,"SELL");addOrder(c,boundaryBuy,listed,buyer,"BUY");addOrder(c,boundarySell,listed,seller,"SELL");trade(c,UUID.randomUUID(),listed,oldBuy,oldSell,1,9,"2026-08-21T15:59:59.500Z");trade(c,UUID.randomUUID(),listed,firstBuy,firstSell,2,11,"2026-08-21T16:00:00Z");trade(c,UUID.randomUUID(),listed,laterBuy,laterSell,3,12,"2026-08-22T01:00:00Z");trade(c,UUID.randomUUID(),listed,boundaryBuy,boundarySell,10,99,"2026-08-22T16:00:00Z");return null;});
            ...
            assertThat(active.latestPrice()).isEqualTo(Money.ofMinor(99));assertThat(active.change()).isEqualTo(Money.ofMinor(90));assertThat(active.volume()).isEqualTo(5);assertThat(active.turnover()).isEqualTo(Money.ofMinor(58));assertThat(active.marketCapitalization()).isEqualTo(Money.ofMinor(99_000));
