...
class PublicStockQueryServiceTest {
    ...
    @Test void market_uses_real_trades_for_shanghai_day_without_mutating_facts() throws Exception {
        ...
        try(Database db=new Database("jdbc:sqlite:"+file)){db.migrate(); CompanyId listed=Fixtures.company(db,100); UUID buyer=UUID.randomUUID(),seller=UUID.randomUUID();
            db.inTransaction(c->{list(c,listed,"BS000001",10,1000);addOrder(c,UUID.randomUUID(),listed,buyer,"BUY");addOrder(c,UUID.randomUUID(),listed,seller,"SELL");return null;});
