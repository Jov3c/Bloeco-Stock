...
class SqlCompanyFinanceRepositoryTest {
    ...
    void stores_capitalization_and_returns_unsettled_operations() throws Exception {
        ...
        try (Database database = new Database("jdbc:sqlite:" + file)) {
            ...
            try(var c=database.dataSource().getConnection();var s=c.prepareStatement("SELECT amount_minor FROM escrow_ledger_entries WHERE liability_kind='COMPANY_TREASURY' AND company_id=?")){s.setString(1,companyId.value().toString());try(var rows=s.executeQuery()){assertThat(rows.next()).isTrue();assertThat(rows.getLong(1)).isEqualTo(7);}}
