...
启动验证通过后，BlockStock 会在 SQL 线程先执行遗留公司资本化恢复，再执行 IPO 认购恢复；两者均成功后才将公司命令切换为可接受状态并启动 IPO 调度。已处于 `ESCROW_DEPOSITED` 的 IPO 认购代表托管入账已持久确认，恢复仅在 SQLite 中恰好一次地补齐公司现金、股份和审计，绝不会再次调用 Vault。`PREPARED` 与 `PLAYER_WITHDRAWN` 无法证明外部结果，会标记为 `AMBIGUOUS`；重复启动不会重复入账、不会重放 Vault 调用。`/company recovery list` 合并显示注册、资本托管和 IPO 三类记录，且是严格只读查询。
