...
function MarketZoneForHour([int]$hour){$offset=$hour-[DateTime]::UtcNow.Hour;while($offset -gt 11){$offset-=24};while($offset -lt -12){$offset+=24};if($offset -eq 0){return 'Etc/GMT'};return 'Etc/GMT'+$(if($offset -gt 0){'-'}else{'+'})+[math]::Abs($offset)}
...
function Start-QA([string]$zone){Set-MarketZone $zone;$script:qaProcess=Start-Process -FilePath $java -ArgumentList @('-Djdk.net.unixdomain.tmpdir=C:\Temp','-Xms512M','-Xmx1G','-jar','paper-1.21.4.jar','--nogui') -WorkingDirectory $qa -WindowStyle Hidden -PassThru;Wait-Port 25566;Wait-Port 25567;Start-Sleep -Seconds 8;$log=(Get-Content (Join-Path $qa 'logs\latest.log') -Tail 240) -join "`n";if($log -match 'BlockStock 启动失败|startup failure'){throw 'BlockStock QA enable failed; inspect qa-server/logs/latest.log'}}
...
try{
  $preZone=MarketZoneForHour 7;$openZone=MarketZoneForHour 12;$postZone=MarketZoneForHour 21
  Start-QA $preZone;Run-Gui 'PREOPEN';Stop-QA
  Start-QA $openZone;Run-Gui 'OPEN';Run-Gui 'POSTSEED';Stop-QA
  Start-QA $postZone;Run-Gui 'POSTCLOSE';Start-Sleep -Seconds 70;Stop-QA
  Start-QA $openZone;Start-Sleep -Seconds 12;Stop-QA
  Set-DividendFixture
  Start-QA $postZone;Start-Sleep -Seconds 70;Stop-QA
  Set-DividendFixture
  Start-QA $postZone;Start-Sleep -Seconds 70;Stop-QA
