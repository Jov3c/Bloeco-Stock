...
根命令 /company 不再返回 Paper 默认 usage；它根据发送者的具体权限给出中文帮助：

- 有创建权限：显示 /company create <公司名> <30|50|70>。
- 有查询权限：显示 /company info <公司名>。
- 有恢复权限：显示 /company recovery list。
- 无任何公司权限：显示已有的中文无权限提示。

TabCompleter 在启动窗口和 ready 状态都注册。它只返回常量候选值，不访问 SQLite：
...
- TDD 覆盖空命令帮助、玩家/管理员根补全、恢复隔离、股息档位与前缀、多词名称、控制台和无权限发送者。
